...
#seed will create same set of random number even if we run the code multiple times. This is very useful when it comes to debugging
from random import seed
#returns a randomly selected element from range(start, stop, step)
from random import randrange
#to read CSV file from dataset
from csv import reader
#squareroot function from math library
...
def load_csv(filename):
	dataset = list()
	#Opening it as readable file
	with open(filename, 'r') as file:
		#initiate csv reder
		csv_reader = 	reader(file)
		#for every row in dataset append to list
...
def string_column_to_float(dataset, column):
	...
	#strip('a') returns the copy of a string by removing both leading and tailing characters i.e, a. if nothing is given in arguments, it ll just removes the whitespaces in the string
	
#convert string column to integer
def string_column_to_int(dataset, column):
	#store given column
	class_column = [row[column] for row in dataset]
	#set function removes duplicates
	unique = set(class_column)
	#initiate lookup table
	lookup = dict()
	for i, value in enumerate(unique):
		#add it to our lookup table
		lookup[value] = i
		#the lookup table stores the pointers to the strings
		for row in dataset:
...
#this function is responsible for making prediction for each decision tree in random forest and returning single prediction from set of predictions
...
def random_forest(train, test, max_depth, min_size, sample_size, n_trees, n_features):
	...
	for i in range(n_trees):
		...
		trees.append(tree)
	predictions = [bagging_predictions(trees, rows) for rows in test]
	
def main():
	seed(1)
	#load and prepare data
	filename = 'pulse_classify.csv'
	dataset = load_csv(filename)
	#converts string attributes to integers
	for i in range(len(datatset)-1):
		string_column_to_float(datatset, i)
	#convert class column to string
	string_column_to_int(dataset, len(dataset[0])-1)
	#evaluate algorithm
	#number of decision trees
	n_folds = 5
	#number of node levels in each decision tree
	max_depth = 10
	min_size = 1
	sample_size = 1.0
...
if __name__ == '__main__':
	import os
	os.chdir('C:\\Users\\Tirumalesh\\Documents\\Python Scripts\\random forest')
	main()
